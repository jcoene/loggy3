namespace :wow do
	...
	task :import => :environment do
		...
		filename, label = ENV['file'], ENV['label']
		...
		fatal "Couldn't import log!" if !log.parse_text_to_new_report(:label => label)
	end
	...
	task :importgz => :environment do
		...
		log = Wow::Importer.new(filename)
		fatal "Couldn't import log!" if !log.parse_gz_to_new_report(:label => label)
	end
